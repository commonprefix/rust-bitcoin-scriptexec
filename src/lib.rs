...
use bitcoin::script::{
    self, Builder, Instruction, Instructions, PushBytes, Script, ScriptBuf, ScriptExt,
};
    ...
    while let Some((pos, mut word)) = words.next() {
        ...
        if let Ok(op) = Opcode::from_str(word) {
            ...
            if op.code <= OP_PUSHDATA4.code {
                let (next, push) = words
                    .next()
                    .ok_or(err(pos, AsmParseErrorKind::UnexpectedEOF))?;
                ...
                let expected_push_op = match buf.len() {
                    n if n < opcodes::all::OP_PUSHDATA1.code as usize => Opcode::from(n as u8),
